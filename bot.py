...
# DATABASE SAFE VERSION (V3 DB COMPATIBLE) - UNTOUCHED LOGIC
# ==========================================
...
async def generate_ai_caption(file_info):
    ...
    # 🌟 ডাইনামিক ডেজ লেফট (বট এবং ওয়েবসাইটে মেয়াদ লাইভ আপডেট হবে)
    expiry_text = ""
    remaining_days = calculate_remaining_days(file_info.get("expiry_date"))
    
    if remaining_days is None:
        remaining_days_text = file_info.get("expiry_raw") or "Unlimited"
        expiry_icon = "🟢"
    else:
        remaining_days_text = f"{remaining_days} Days"
        if remaining_days <= 1:
            expiry_icon = "🔴"
    ...
    expiry_text = f"\n┣ {expiry_icon} <b>মেয়াদ:</b> <code>{remaining_days_text}</code>"
...
async def bot_health_check():
    ...
    try:
        ...
        logger.info("✅ Mongo indexes ensured (TTL Active)")
    except Exception as e:
...
# V3 DATABASE SAFE VERSION - UNTOUCHED LOGIC
# ==========================================
...
# ==========================================
# 🐛 BUG FIX: PROCESS CUSTOM EXPIRY TEXT
# ==========================================

async def process_expiry_text(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE
):
    ...
    if not expiry_date:
        await update.message.reply_text(
            "❌ <b>Invalid format. Try again (e.g., 7 days, 1 month).</b>",
            parse_mode="HTML",
        )
        return ASK_EXPIRY
...
async def process_custom_msg(
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton("📥 Both (File + Web)", callback_data="ptype_both")
        ],
    ...
    await context.bot.send_message(
        ...
        text="📢 <b>চ্যানেলে কীভাবে পোস্ট করতে চান সিলেক্ট করুন:</b>\n\n"
             ...
             "৩. <b>Both:</b> ক্যাপশন সহ ফাইল এবং লিংক দুটিই থাকবে।",
        parse_mode="HTML",
...
async def process_post_type(
    ...
    post_type = query.data.replace("ptype_", "")
...
async def post_single_file(
    ...
    try:
        ...
        post_type = working_doc.get("post_type", "both") 
        file_id = working_doc["id"]

        web_link = f"{WEBSITE_DOMAIN}/config/{working_doc['uid']}"
        direct_link = f"https://t.me/{context.bot.username}?start=get_{working_doc['uid']}"
...
async def handle_start(
    ...
    if "get_" in text:
        ...
        # 🌟 মেয়াদ শেষ হয়েছে কি না ডাইনামিক চেক 🌟
        days_left = calculate_remaining_days(file_doc.get("expiry_date"))
        if days_left is not None and days_left <= 0:
            await update.message.reply_text(
                "❌ <b>ফাইলটির মেয়াদ শেষ হয়ে গেছে!</b>",
                parse_mode="HTML",
            )
            return
...
async def db_reset_callback(
    ...
    if query.data == "db_reset_confirm":
        ...
        await query.edit_message_text(
            "❌ <b>ডাটাবেস রিসেট বাতিল করা হয়েছে।</b> আপনার ফাইলগুলো নিরাপদ আছে।", 
            parse_mode="HTML"
        )
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            ASK_EXPIRY: [
                ...
                # 🌟 এই বাগ ফিক্সটি এখানে যোগ করা হয়েছে 🌟
                MessageHandler(
                    filters.TEXT & ~filters.COMMAND,
                    process_expiry_text
                )
